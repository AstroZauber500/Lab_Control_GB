...
The IMC-PID controller met both specifications experimentally 
($t_s = 5.00$~s, $M_p = 2.7\%$) despite the simulation marginally 
violating them ($t_s = 5.01$~s, $M_p = 3.4\%$), as the IMC tuning 
was deliberately placed near the stability boundary to compensate 
for the plant's inherent overshoot. The principal limitation of 
this design is structural: the derivative action operates on the 
error signal, so the step reference excites the derivative channel 
and produces a pronounced control effort spike near steady state, 
amplifying the plant's lightly damped resonance ($\zeta \approx 0.029$). 
To mitigate this, the derivative filter coefficient was reduced from 
$N = 3$ to $N = 5$ on the physical platform, attenuating the 
high-frequency content injected by quantisation and unmodelled 
dynamics. 
