...
%     G3_approx(s).
% -----------------------------------------------------------------
The logarithmic decrement method was applied to each trial to extract $\omega_n$, $\zeta$, and the static gain $K$ from the measured transient features (successive peak amplitudes and oscillation period):
...
% -----------------------------------------------------------------
MATLAB's \textit{ident} toolbox was used in parallel to fit the same second-order prototype to each dataset via prediction-error minimization, producing three additional transfer functions with their respective best-fit percentages.
...
%     remaining 4 TFs.
% -----------------------------------------------------------------
The six identified models were pooled and their parameters compared statistically. The mean $\bar{x}$ and sample standard deviation $s$ were computed for $K$, $\zeta$, and $\omega_n$. The two models derived from trial~3 showed parameter deviations exceeding $2s$ from the group mean, consistent with an external disturbance during that recording session. Those two models were therefore excluded, and the final plant was obtained by averaging the parameters of the four remaining transfer functions:

\begin{equation}
  G(s) = \frac{K_{\text{avg}}\,\omega_{n,\text{avg}}^2}
              {s^2 + 2\,\zeta_{\text{avg}}\,\omega_{n,\text{avg}}\,s + \omega_{n,\text{avg}}^2}
  \label{eq:plant_final}
\end{equation}

% REPLACE with numerical values once computed:
 \begin{equation}
   G(s) = \frac{5.287}{s^2 + 0.2481s + 15.43}
 \end{equation}

% -----------------------------------------------------------------
% SUGGESTED TABLE — statistical summary (6 TFs vs. 4 valid TFs):
% \begin{table}[H]
%   \centering
%   \caption{Statistical summary of identified parameters}
%   \begin{tabular}{lcccc}
%     \toprule
%     Param. & $\bar{x}$ (all 6) & $s$ (all 6) & $\bar{x}$ (4 valid) & $s$ (4 valid) \\
%     \midrule
%     $K$        & ... & ... & ... & ... \\
%     $\zeta$    & ... & ... & ... & ... \\
%     $\omega_n$ & ... & ... & ... & ... \\
...
% \end{table}
%
% SUGGESTED FIGURE — 4 valid TF responses + averaged model vs.
% experimental data from trials 1 and 2:
% \begin{figure}[H]
%   \centering
%   \includegraphics[width=0.45\textwidth]{plant_final.png}
%   \caption{Step responses of the four valid models, the averaged
%            plant $G(s)$, and the experimental data from trials 1
%            and 2.}
%   \label{fig:plant}
% \end{figure}
% -----------------------------------------------------------------
...
Once the IMC-equivalent open-loop transfer function $C(s)\,G(s)$ was obtained, its numerator and
denominator polynomials were extracted in MATLAB.Identifying the coefficients
of \eqref{eq:pid_continuous} with those polynomials, and setting $N$ equal to the second denominator coefficient.
The tuning procedure yielded the following parameter values:
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.45\textwidth]{Figures/simulation_PID.png}
